...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            origin_values = self.values.copy()
            for state in self.mdp.getStates():
                self.values[state] = max([self.computeQValueFromValues(state, action, origin_values) for action in
                                          self.mdp.getPossibleActions(state)], default=self.values[state])
    ...
    def computeQValueFromValues(self, state, action, vals=None):
        """
        ...
        for next_state, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            weighted += (probability * (
                    self.mdp.getReward(state, action, next_state) + (self.discount * vals[next_state])))
        return weighted
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def getMaxQ(self, state):
        ...
        count_q_vals = util.Counter(
            {action: self.computeQValueFromValues(state, action) for action in possible_actions})
        if len(count_q_vals) > 0:
            return count_q_vals[count_q_vals.argMax()]
        else:
            return None
