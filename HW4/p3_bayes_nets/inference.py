...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        evidence_variables = set(evidenceDict.keys())
        query_variables = set(queryVariables)

        current_factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationVariables in eliminationOrder:
            current_factors, joined_factors = joinFactorsByVariable(current_factors, eliminationVariables)
            if len(joined_factors.unconditionedVariables()) != 1:
                current_factors.append(eliminate(joined_factors, eliminationVariables))
        if len(current_factors) == 1:
            return normalize(current_factors[0])
        else:
