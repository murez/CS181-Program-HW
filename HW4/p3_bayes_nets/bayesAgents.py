...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obs_var = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obs_var)
            edges.append((FOOD_HOUSE_VAR, obs_var))
            edges.append((GHOST_HOUSE_VAR, obs_var))
            variableDomainsDict[obs_var] = OBS_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    for (val, housePos) in [(BOTTOM_LEFT_VAL, bottomLeftPos),
                            (TOP_LEFT_VAL, topLeftPos),
                            (BOTTOM_RIGHT_VAL, bottomRightPos),
                            (TOP_RIGHT_VAL, topRightPos)]:
        for obsPos in gameState.getHouseWalls(housePos):
            obs_var = OBS_VAR_TEMPLATE % obsPos
            obs_factor = bn.Factor([obs_var], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in obs_factor.getAllPossibleAssignmentDicts():
                food = assignment[FOOD_HOUSE_VAR] == val
                ghost = assignment[GHOST_HOUSE_VAR] == val
                prob = 0
                if food:
                ...
                elif not food and not ghost:
                    if assignment[obs_var] == NO_OBS_VAL:
                        prob = 1

                obs_factor.setProbability(assignment, prob)
            bayesNet.setCPT(obs_var, obs_factor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    return max(AssignmentDictList, key=lambda x: factor.getProbability(x))
